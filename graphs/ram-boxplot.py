...
for box, ran, color, hatch in zip(b['boxes'], rans, cycle(colors), cycle(['/', ''])):
    ax.add_patch(Polygon(box.get_xydata(), facecolor=color, label=ran, hatch=hatch))

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Comsumo de memória no núcleo (GB)', fontsize=14)
x = np.arange(len(labels)) * 4 + 2.5
